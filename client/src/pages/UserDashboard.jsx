      ...
      <main className="flex-1 p-6 md:p-8 overflow-y-auto">
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-3 sm:space-y-0">
            ...
            <Link
              to="/complaints/new"
              className="bg-gov-dark hover:bg-gov-accent text-white font-bold text-xs py-2.5 px-4 rounded-full shadow-sm hover:shadow flex items-center space-x-2 transition"
            >
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            {/* Stat Item 2 */}
            <div className="surface-panel p-5 rounded-[1.25rem] flex items-center space-x-4">
              <div className="bg-[#e8ddc9] p-3 rounded-2xl text-gov-dark">
          ...
          <div className="surface-panel rounded-[1.5rem] overflow-hidden p-6 space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="relative max-w-sm w-full">
                ...
                <input
                  ...
                  className="block w-full pl-10 pr-3 py-2 text-sm border border-[#d8cbb8] rounded-full focus:outline-none focus:border-gov-dark bg-white/80 focus:bg-white text-slate-800 font-medium transition"
                />
              </div>

              {/* Tab Filters */}
              <div className="flex flex-wrap gap-1.5 border-b md:border-b-0 border-[#d8cbb8] pb-2 md:pb-0">
                {["ALL", "PENDING", "ACTIVE", "RESOLVED", "REJECTED"].map((tab) => (
                  <button
                    ...
                    className={`text-xs font-bold px-3 py-2 rounded-lg transition ${
                      activeTab === tab
                        ? "bg-gov-dark text-white"
                        : "bg-white/70 text-slate-600 hover:bg-[#e9dfcf]"
                    }`}
                  >
            ...
            {loading ? (
              <div className="py-20 flex flex-col items-center justify-center space-y-4">
                <div className="w-10 h-10 border-4 border-gov-dark border-t-transparent rounded-full animate-spin"></div>
                <p className="text-xs text-slate-500 font-medium">Loading Grievances...</p>
              </div>
            ) : filteredComplaints.length === 0 ? (
              <div className="py-20 text-center border border-dashed border-[#d8cbb8] rounded-[1.5rem] bg-white/50 flex flex-col items-center justify-center p-6">
